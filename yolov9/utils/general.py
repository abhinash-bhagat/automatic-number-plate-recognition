...
def increment_path(path, exist_ok=False, sep='', suffix=''):
    # Increment file path, i.e. runs/exp --> runs/exp{sep}2{suffix}, runs/exp{sep}3{suffix}, ... etc.
    path = Path(path)  # os-agnostic
    if path.exists() and not exist_ok:
        base_path, file_name = path.parent, path.stem
        for n in range(2, 9999):
            p = f'{base_path / file_name}{sep}{n}{suffix}'  # increment path
            if not os.path.exists(p):  
                break
        path = Path(p)
    return path
